...
import { triggerStep } from "./steps/trigger.js";
import { getErrorMessageAsync } from "./utils.js";
...
export function getStepImporter(actionType: string): StepImporter | undefined {
  return getPluginRegistry().stepImporters[actionType];
}
...
export function getActionLabel(actionType: string): string | undefined {
  return getPluginRegistry().actionLabels[actionType];
}
...
async function executeActionStep(input: {
  ...
  if (actionType === "Loop") {
    const systemAction = SYSTEM_ACTIONS.Loop;
    const module = await systemAction.importer();
