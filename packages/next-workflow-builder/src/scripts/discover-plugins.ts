...
async function updateReadme(): Promise<void> {
  ...
  const { getAllIntegrations } = await importPackageOrLocal("registry.ts") as { getAllIntegrations: () => { label: string; actions: { label: string }[] }[] };
...
async function generateTypesFile(): Promise<void> {
  ...
  const { getIntegrationTypes } = await importPackageOrLocal("registry.ts") as { getIntegrationTypes: () => string[] };
...
async function processStepFilesForCodegen(): Promise<void> {
  const { getAllIntegrations, computeActionId } = await importPackageOrLocal("registry.ts") as {
    getAllIntegrations: () => { type: string; actions: { stepImportPath: string; stepFunction: string; slug: string }[] }[];
...
async function generateStepRegistry(): Promise<void> {
  const { getAllIntegrations, computeActionId } = await importPackageOrLocal("registry.ts") as {
    getAllIntegrations: () => { type: string; label: string; actions: { slug: string; label: string; stepImportPath: string; stepFunction: string }[] }[];
    computeActionId: (type: string, slug: string) => string;
  };
  const { LEGACY_ACTION_MAPPINGS } = await importPackageOrLocal("legacy-mappings.ts") as {
    LEGACY_ACTION_MAPPINGS: Record<string, string>;
  };
  const integrations = getAllIntegrations();
...
async function generateOutputDisplayConfigs(): Promise<void> {
  const { getAllIntegrations, computeActionId } = await importPackageOrLocal("registry.ts") as {
    getAllIntegrations: () => { type: string; actions: { slug: string; outputConfig?: { type: string; field: string } }[] }[];
    computeActionId: (type: string, slug: string) => string;
  };
  const integrations = getAllIntegrations();
