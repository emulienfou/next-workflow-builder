...
export function ConfigurationOverlay({ overlayId }: ConfigurationOverlayProps) {
  ...
  const handleCopyCode = useCallback(() => {
    if (!selectedNode) return;
    navigator.clipboard.writeText(generateNodeCode(selectedNode, currentWorkflowId!));
    ...
    <div className="flex h-full max-h-[80vh] flex-col">
      ...
      <div className="flex-1 overflow-y-auto">
        { activeTab === "properties" && (
          <div className="space-y-4 px-6 pt-4 pb-6">
            {/* Action selection */ }
            { selectedNode.data.type === "action" && !selectedNode.data.config?.actionType && isOwner && (
              <ActionGrid
            ...
            { selectedNode.data.type === "action" && selectedNode.data.config?.actionType !== undefined && (
              <ActionConfig
                config={ selectedNode.data.config || {} }
                disabled={ isGenerating || !isOwner }
                isOwner={ isOwner }
            ...
            { isOwner && (
              <div className="flex items-center gap-2 pt-2">
                { selectedNode.data.type === "action" && (
                  <Button className="text-muted-foreground" onClick={ handleToggleEnabled } size="sm" variant="ghost">
                ...
                <Button className="text-muted-foreground" onClick={ handleDeleteNode } size="sm" variant="ghost">
                  <Trash2 className="mr-2 size-4"/>
                  Delete
                </Button>
              </div>
        ...
        { showCodeTab && (
          <div className={ `flex flex-col ${ activeTab === "code" ? "" : "invisible absolute -z-10" }` }>
            <div className="flex shrink-0 items-center justify-between border-b bg-muted/30 px-3 py-2">
              <div className="flex items-center gap-2">
                <FileCode className="size-3.5 text-muted-foreground"/>
                <code className="text-muted-foreground text-xs">{ getCodeFilename(selectedNode) }</code>
              </div>
              <Button className="text-muted-foreground" onClick={ handleCopyCode } size="sm" variant="ghost">
            ...
            <div className="h-[400px]">
              <CodeEditor
                ...
                language={
                  (
                    selectedNode.data.type === "trigger" &&
                    (selectedNode.data.config?.triggerType as string) === "Schedule"
                  ) ?
                    "json"
                    : "typescript"
                }
                ...
                value={ generateNodeCode(selectedNode, currentWorkflowId!) }
              />
            </div>
          </div>
        ) }
        ...
        { activeTab === "runs" && isOwner && (
          <div className="flex h-full flex-col">
            <div className="flex shrink-0 items-center gap-2 border-b px-4 py-2">
              ...
              <Button className="text-muted-foreground" onClick={ handleDeleteAllRuns } size="sm" variant="ghost">
                <Eraser className="mr-2 size-4"/>
                Clear All
              </Button>
            </div>
      ...
      <div className="flex shrink-0 items-center justify-around border-t bg-background pb-safe">
        <button
          className={ `flex flex-1 flex-col items-center gap-1 py-3 font-medium text-xs transition-colors ${
            activeTab === "properties" ? "text-foreground" : "text-muted-foreground"
          }` }
          onClick={ () => setActiveTab("properties") }
          type="button"
        >
