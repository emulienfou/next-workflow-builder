...
function useAllActions(): Omit<ActionType, "codeGenerator" | "icon">[] {
  return useMemo(() => {
    const pluginActions = getAllActions();

    // Map plugin actions to ActionType format
    const mappedPluginActions: Omit<ActionType, "codeGenerator" | "icon">[] = pluginActions.map((action) => ({
      id: action.id,
      label: action.label,
...
function GroupIcon({
  ...
  group: { category: string; actions: Omit<ActionType, "codeGenerator" | "icon">[] };
}) {
...
function ActionIcon({
  ...
  action: Omit<ActionType, "codeGenerator" | "icon">;
  className?: string;
}) {
...
export function ActionGrid({
  ...
  const groupedActions = useMemo(() => {
    const groups: Record<string, Omit<ActionType, "codeGenerator" | "icon">[]> = {};
